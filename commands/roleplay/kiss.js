...
module.exports = {
    ...
    async execute(message, args) {
        let hug, target;
        await fetchRandom('kiss').then(obj => hug = obj.results[0]).catch(error => console.log(error));
        if(!hug) return message.channel.send('API request failure.');
        const list = [];
        if(args) {
            for ( let i = 0; i < args.length; i++){
                await getUser(args[i], message).then(trgt => {
                    if (trgt && target !== undefined){
                        list.push(`<@!${trgt.id}>`);
                    }
        ...
        const emb = {
            color: 15277667,
            title: '',
            description: `<@!${message.author.id}> spreads around kisses`,
            image: {
                url: `${hug.url}`
            },
            fields: [],
        ...
        if (list.length) {
            emb.description = `<@!${message.author.id}> gives *passionate* kisses to ${list.join(', ')}.`
        }
        try {
