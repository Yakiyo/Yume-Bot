...
module.exports = {
    ...
    async execute(message, args) {
        let hug;
        await fetchRandom('slap').then(obj => hug = obj.results[0]).catch(error => console.log(error));
        ...
        const emb = {
            color: 15277667,
            title: '',
            description: `<@!${message.author.id}> slaps`,
            image: {
                url: `${hug.url}`,
            },
            fields: [],
        ...
        if (list.length) {
            emb.description = `<@!${message.author.id}> slaps ${list.join(', ')} hard.`;
        }
        try {
