...
module.exports = {
    name: 'cuddle',
    description: 'Gives a cuddle gif',
    ...
    async execute(message, args) {
        let hug;
        await fetchRandom('cuddle').then(obj => hug = obj.results[0]).catch(error => console.log(error));
        ...
        const emb = {
            color: 15277667,
            title: 'Cuddles',
            description: `<@!${message.author.id}> needs some cuddling`,
            image: {
                url: `${hug.url}`,
            },
            fields: [],
        ...
        if (list.length) {
            emb.description = `<@!${message.author.id}> is cuddling with ${list.join(', ')}.`;
        }
        try {
